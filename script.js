...
/* Exercise #3: Create a function that takes the following parameters:
                ...
                (For this exercise please use traditional loops (not array methods), 
                for-loop, while loop, or for-of loop)
*/
...
function tripleArrayValues(arr, num) {
  let tripleArray = [];
  for (let i = 0; i < arr.length; i++) {
    triplenums = arr[i] * num;
    tripleArray.push(arr[i] * num);
    console.log(triplenums);
  }
  console.log(tripleArray);
}

tripleArrayValues(nums, 3);
tripleArrayValues(nums2, 3);
